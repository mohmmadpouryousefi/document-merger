...
class TestAssertions:
    ...
    def assert_pdf_valid(test_case, filepath):
        ...
        test_case.assertTrue(os.path.exists(filepath), f"PDF file should exist: {filepath}")
        test_case.assertGreater(os.path.getsize(filepath), 0, "PDF file should not be empty")
        
        try:
            import PyPDF2
            with open(filepath, 'rb') as f:
                reader = PyPDF2.PdfReader(f)
                test_case.assertGreater(len(reader.pages), 0, "PDF should have at least one page")
        except ImportError:
            # If PyPDF2 not available, just check file size
            test_case.assertGreater(os.path.getsize(filepath), 100, "PDF file should be reasonable size")
    
    @staticmethod
    def assert_excel_valid(test_case, filepath, expected_sheets=None):
        """Assert that an Excel file is valid and readable."""
        test_case.assertTrue(os.path.exists(filepath), f"Excel file should exist: {filepath}")
        test_case.assertGreater(os.path.getsize(filepath), 0, "Excel file should not be empty")
        
        workbook = openpyxl.load_workbook(filepath)
        test_case.assertGreater(len(workbook.sheetnames), 0, "Excel should have at least one sheet")
        
        if expected_sheets:
            test_case.assertEqual(len(workbook.sheetnames), expected_sheets, 
