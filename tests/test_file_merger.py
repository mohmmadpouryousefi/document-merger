...
class TestFileMerger(unittest.TestCase):
    ...
    def test_merge_pdf_files(self, mock_detector_class):
        ...
        mock_detector.validate_files.return_value = ('pdf', ['file1.pdf', 'file2.pdf'], [])
        mock_detector.is_file_accessible.return_value = True
        
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'validate_files', return_value=('pdf', ['file1.pdf', 'file2.pdf'], [])), \
             patch.object(self.merger.detector, 'is_file_accessible', return_value=True), \
             patch.object(self.merger, '_validate_file_integrity', return_value=True), \
             patch.object(self.merger.pdf_merger, 'merge_pdfs', return_value=True) as mock_merge:
    ...
    def test_merge_pdf_files_with_bookmarks(self, mock_detector_class):
        ...
        mock_detector = mock_detector_class.return_value
        mock_detector.validate_files.return_value = ('pdf', ['chapter1.pdf', 'chapter2.pdf'], [])
        mock_detector.is_file_accessible.return_value = True
        
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'validate_files', return_value=('pdf', ['chapter1.pdf', 'chapter2.pdf'], [])), \
             patch.object(self.merger.detector, 'is_file_accessible', return_value=True), \
             patch.object(self.merger, '_validate_file_integrity', return_value=True), \
             patch.object(self.merger.pdf_merger, 'merge_with_bookmarks', return_value=True) as mock_merge:
            ...
            self.assertTrue(result['success'])
            mock_merge.assert_called_once_with(['chapter1.pdf', 'chapter2.pdf'], 'book.pdf')
    
    @patch('core.file_merger.FileTypeDetector')
    def test_merge_excel_files(self, mock_detector_class):
        """Test Excel file merging through orchestrator."""
        mock_detector = mock_detector_class.return_value
        mock_detector.validate_files.return_value = ('excel', ['data1.xlsx', 'data2.xlsx'], [])
        mock_detector.is_file_accessible.return_value = True
        
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'validate_files', return_value=('excel', ['data1.xlsx', 'data2.xlsx'], [])), \
             patch.object(self.merger.detector, 'is_file_accessible', return_value=True), \
             patch.object(self.merger, '_validate_file_integrity', return_value=True), \
             patch.object(self.merger.excel_merger, 'merge_excel_files', return_value=True) as mock_merge:
            ...
            self.assertTrue(result['success'])
            mock_merge.assert_called_once_with(['data1.xlsx', 'data2.xlsx'], 'combined.xlsx')
    
    @patch('core.file_merger.FileTypeDetector')
    def test_unsupported_file_type(self, mock_detector_class):
        """Test handling of unsupported file types."""
        mock_detector = mock_detector_class.return_value
        mock_detector.validate_files.return_value = ('txt', [], ['Unsupported file type: txt'])
        
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'validate_files', return_value=('txt', [], ['Unsupported file type: txt'])):
    ...
    def test_mixed_file_types(self, mock_detector_class):
        ...
        mock_detector = mock_detector_class.return_value
        mock_detector.validate_files.return_value = (None, [], ['All files must be of the same type. Found: excel, pdf'])
        
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'validate_files', return_value=(None, [], ['All files must be of the same type. Found: excel, pdf'])):
    ...
    def test_validate_files_success(self, mock_detector_class):
        ...
        mock_detector = mock_detector_class.return_value
        mock_detector.validate_files.return_value = ('pdf', ['valid1.pdf', 'valid2.pdf'], [])
        mock_detector.is_file_accessible.return_value = True
    ...
    def test_validate_files_invalid_pdf(self, mock_detector_class):
        ...
        mock_detector = mock_detector_class.return_value
        mock_detector.validate_files.return_value = ('pdf', ['valid.pdf'], ['Invalid PDF file: invalid.pdf'])
    ...
    def test_validate_excel_files(self, mock_detector_class):
        ...
        with patch.object(self.merger.detector, 'validate_files', return_value=('excel', ['data1.xlsx', 'data2.xlsx'], [])), \
             patch.object(self.merger.detector, 'is_file_accessible', return_value=True), \
             patch.object(self.merger, '_validate_file_integrity', return_value=True), \
             patch.object(self.merger.excel_merger, 'merge_excel_files', return_value=True):
    ...
    def test_get_pdf_file_info(self, mock_detector_class):
        ...
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'detect_file_type', return_value='pdf'), \
             patch.object(self.merger.detector, 'get_file_size', return_value=1024), \
             patch.object(self.merger.detector, 'format_file_size', return_value='1.0 KB'), \
             patch.object(self.merger.detector, 'is_file_accessible', return_value=True), \
             patch.object(self.merger.pdf_merger, 'get_pdf_info', return_value={'pages': 5, 'title': 'Test Doc'}):
    ...
    def test_get_excel_file_info(self, mock_detector_class):
        ...
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'detect_file_type', return_value='excel'), \
             patch.object(self.merger.detector, 'get_file_size', return_value=2048), \
             patch.object(self.merger.detector, 'format_file_size', return_value='2.0 KB'), \
             patch.object(self.merger.detector, 'is_file_accessible', return_value=True), \
             patch.object(self.merger.excel_merger, 'get_excel_info', return_value={'sheets': 3, 'sheet_names': ['Sheet1', 'Sheet2', 'Sheet3']}):
...
class TestFileMergerErrorHandling(unittest.TestCase):
    ...
    def test_merge_files_detector_error(self, mock_detector_class):
        ...
        # Mock the detector that's created in FileMerger.__init__
        with patch.object(self.merger.detector, 'validate_files', side_effect=Exception("Detector error")):
    ...
    def test_merge_files_merger_error(self, mock_detector_class):
        ...
        with patch.object(self.merger.pdf_merger, 'validate_pdf', return_value=True), \
             patch.object(self.merger.pdf_merger, 'merge_pdfs', side_effect=Exception("Merge error")):
