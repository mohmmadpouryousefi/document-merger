...
class FileMerger:
    ...
    def merge_files(self, input_files: List[str], output_file: str, 
                   add_bookmarks: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if output_path.is_dir() or not output_path.suffix:
                output_file = self._generate_output_filename(accessible_files, file_type, str(output_path))
                result['output_file'] = output_file
            
            # Perform the merge based on file type
            if file_type == 'pdf':
                success = self._merge_pdf_files(accessible_files, output_file, add_bookmarks)
            ...
            if success:
                result['success'] = True
                result['message'] = f"Successfully merged {len(accessible_files)} {file_type} files"
    ...
    def _merge_pdf_files(self, files: List[str], output_file: str, add_bookmarks: bool) -> bool:
        """Merge PDF files."""
        if add_bookmarks:
    ...
    def _generate_output_filename(self, input_files: List[str], file_type: str, 
    ...
    def get_file_info(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            basic_info = {
                ...
                'size_formatted': self.detector.format_file_size(self.detector.get_file_size(file_path)),
                'type': file_type,
    ...
    def reorder_files(self, files: List[str], new_order: List[int]) -> List[str]:
        ...
        try:
            ...
            if set(new_order) != set(range(len(files))):
                raise ValueError("New order must contain all indices from 0 to len(files)-1")
