...
class FileManagerGUI:
    ...
    def _create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="File Merger", font=('Arial', 16, 'bold'))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 20))
        
        # File selection frame
        file_frame = ttk.LabelFrame(main_frame, text="Select Files to Merge", padding="10")
        file_frame.grid(row=1, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        file_frame.columnconfigure(1, weight=1)
        
        # File selection buttons
        ttk.Button(file_frame, text="Add Files", command=self._add_files).grid(row=0, column=0, padx=(0, 10))
        ttk.Button(file_frame, text="Remove Selected", command=self._remove_selected).grid(row=0, column=1, padx=5)
        ttk.Button(file_frame, text="Clear All", command=self._clear_files).grid(row=0, column=2, padx=5)
        ttk.Button(file_frame, text="Move Up", command=self._move_up).grid(row=0, column=3, padx=5)
        ttk.Button(file_frame, text="Move Down", command=self._move_down).grid(row=0, column=4, padx=(5, 0))
        ...
        self.info_text = scrolledtext.ScrolledText(info_frame, height=6, state=tk.DISABLED)
        ...
        ttk.Checkbutton(options_frame, text="Add bookmarks to PDF (recommended)", 
                       variable=self.add_bookmarks_var).grid(row=0, column=0, sticky=tk.W)
        ...
        ttk.Button(action_frame, text="Preview", command=self._preview_merge).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(action_frame, text="Merge Files", command=self._merge_files, 
    ...
    def _update_file_info(self):
        ...
        if not self.selected_files:
            ...
            file_type, valid_files, errors = self.detector.validate_files(self.selected_files)
    ...
    def _preview_merge(self):
        ...
        if preview['errors']:
            ...
            for i, file_info in enumerate(preview['files_info'], 1):
                ...
                if preview['file_type'] == 'pdf' and 'pages' in file_info:
                    ...
                    if 'sheet_names' in file_info:
                        content += f"   Sheet names: {', '.join(file_info['sheet_names'])}\n"
        ...
        ttk.Button(preview_window, text="Close", 
                  command=preview_window.destroy).pack(pady=(0, 10))
    ...
    def _show_validation_results(self, results):
        ...
        ttk.Button(results_window, text="Close", 
                  command=results_window.destroy).pack(pady=(0, 10))
